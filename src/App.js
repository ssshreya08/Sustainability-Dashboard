    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-6xl rounded shadow-lg p-4 max-h-[85vh] overflow-auto">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {pageRows.map((r, i) => (
              <tr key={(page - 1) * pageSize + i} className={i % 2 === 0 ? "bg-gray-50" : ""}>
                <td>{r.date}</td>
                <td>{r.department}</td>
                <td>{r.unit}</td>
                <td>{r.machine}</td>
        ...
        <div className="mt-3 flex justify-end gap-2">
          <button className="px-3 py-1 border rounded" onClick={() => exportCSV(pageRows, `drilldown-page-${page}.csv`)}>
            Export Page CSV
          </button>
        </div>
      </div>
...
async function exportPdf(elementSelector = ".dashboard-root", fileName = "dashboard.pdf") {
...
function App() {
  ...
  const [filters, setFilters] = useState(() => {
    try {
      ...
      return s ? JSON.parse(s) : { from: null, to: null, department: null, unit: null, machine: null, shift: null };
    } catch {
      return { from: null, to: null, department: null, unit: null, machine: null, shift: null };
    }
  });
  ...
  useEffect(() => localStorage.setItem("sd_tasks", JSON.stringify(tasks)), [tasks]);
  useEffect(() => localStorage.setItem("sd_notes", JSON.stringify(notes)), [notes]);
  useEffect(() => localStorage.setItem("sd_events", JSON.stringify(events)), [events]);
  useEffect(() => localStorage.setItem("sd_audit", JSON.stringify(audit)), [audit]);
  ...
  useEffect(() => {
    ...
    const fd = src.filter((rec) => {
      if (filters.from && new Date(rec.date) < new Date(filters.from)) return false;
      ...
      if (filters.shift && String(rec.shift) !== String(filters.shift)) return false;
      return true;
    });
    setFilteredData(fd);
  }, [filters, data]);
  ...
  const energySeries = useMemo(() => aggregateByDate(filteredData.length ? filteredData : data, "energy"), [filteredData, data]);
  const waterSeries = useMemo(() => aggregateByDate(filteredData.length ? filteredData : data, "water"), [filteredData, data]);
  const wasteSeries = useMemo(() => aggregateByDate(filteredData.length ? filteredData : data, "waste"), [filteredData, data]);
  const emissionsSeries = useMemo(() => aggregateByDate(filteredData.length ? filteredData : data, "emissions"), [filteredData, data]);

  const alerts = useMemo(() => {
    const list = [];
    const source = filteredData.length ? filteredData : data;
    source.forEach((r) => {
      if (r.energy > 180) list.push({ date: r.date, department: r.department, machine: r.machine, metric: "energy", value: r.energy, message: "High energy usage" });
      if (r.emissions > 120) list.push({ date: r.date, department: r.department, machine: r.machine, metric: "emissions", value: r.emissions, message: "High emissions" });
    });
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      const newRec = {
        ...
        shift: options.shifts[Math.floor(Math.random() * options.shifts.length)] || 1,
        department: options.departments[Math.floor(Math.random() * options.departments.length)] || "Unit",
        unit: options.units[Math.floor(Math.random() * options.units.length)] || "Unit A",
        machine: options.machines[Math.floor(Math.random() * options.machines.length)] || "Machine 1",
  ...
  function createTaskFromAlert(alert) {
    const task = { id: Date.now(), title: `${alert.metric} spike`, details: alert, status: "Open", createdAt: new Date().toISOString() };
  ...
  function handleAcknowledge(alert) {
    setAcknowledged((prev) => {
      const next = [...prev, `${alert.metric}|${alert.date}|${alert.machine}|${alert.department}|${alert.value}`];
  ...
  function addNote(key, text) {
    setNotes((n) => {
      const next = { ...n, [key]: [...(n[key] || []), { id: Date.now(), text, ts: new Date().toISOString() }] };
  ...
  function openDrilldownForDate(date) {
    const rows = (filteredData.length ? filteredData : data).filter((r) => r.date === date);
  ...
  const GOALS = { energy: 50000, water: 200000, waste: 20000, emissions: 40000 };
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 dashboard-root">
      <header className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Records loaded: {currentRows.length}</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              onClick={() => {
                setAutoRefresh((v) => !v);
                logAction(autoRefresh ? "auto-refresh stopped" : "auto-refresh started");
              }}
            >
              {autoRefresh ? "Stop Auto-refresh" : "Start Auto-refresh"}
            </button>

            <button className="px-3 py-1 border rounded" onClick={manualRefresh}>
              Refresh
            </button>
            ...
            <button className="px-3 py-1 border rounded" onClick={() => { exportExcel(currentRows, "sustainability_export.xlsx"); logAction("export xlsx"); }}>
              Export Excel
            </button>

            <button className="bg-gray-700 text-white px-3 py-1 rounded" onClick={() => { exportPdf(); logAction("export pdf"); }}>
              Export PDF
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <div>
            ...
            <OverallTile totals={totals} onClick={() => navigate("/insights/overall")} />
          </div>

          <div className="bg-white p-4 rounded shadow text-center">
            <h2 className="text-gray-500">Energy</h2>
            <p className="text-2xl font-bold text-blue-600">{totals.energy} kWh</p>
          </div>
          ...
          <div className="bg-white p-4 rounded shadow text-center">
            ...
            <p className="text-2xl font-bold text-green-600">{totals.emissions} CO₂</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <AlertsPanel
            alerts={alerts.filter((a) => !acknowledged.includes(`${a.metric}|${a.date}|${a.machine}|${a.department}|${a.value}`))}
        ...
        <div className="flex gap-4 items-start">
          <div className="flex-1">
            ...
            <div className="flex gap-2 items-center mb-2">
              <input id="eventDate" type="date" className="border px-2 py-1 rounded" />
              <input id="eventLabel" type="text" placeholder="Event label" className="border px-2 py-1 rounded" />
              <button
          ...
          <div className="w-80">
            ...
            <div className="mb-2">
              <input id="noteKey" placeholder="key e.g. 2025-09-01 or energy" className="border px-2 py-1 rounded w-full mb-1" />
              <textarea id="noteText" placeholder="Add note..." className="border px-2 py-1 rounded w-full mb-1" rows={2} />
                    ...
                    <ul className="text-sm">
                      {arr.map((n) => (
                        <li key={n.id} className="bg-gray-50 p-1 rounded mb-1">
                          {n.text} <span className="text-xs text-gray-400"> — {new Date(n.ts).toLocaleString()}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <ResponsiveContainer>
                {chartType === "line" ? (
                  <LineChart data={energySeries}>
                    ...
                    <Tooltip />
                    {events.map((ev) => (
                      <ReferenceLine key={ev.id} x={ev.date} stroke="red" label={ev.label} />
                    ))}
                    <Line type="monotone" dataKey="energy" stroke="#3b82f6" dot={{ r: 3 }} onClick={(e) => { if (!e?.payload) return; openDrilldownForDate(e.payload.date); }} />
                  </LineChart>
                ) : (
                  <BarChart data={energySeries}>
                    ...
                    <Bar dataKey="energy" fill="#3b82f6" onClick={(e) => e?.payload && openDrilldownForDate(e.payload.date)} />
                  </BarChart>
                )}
              </ResponsiveContainer>
            </div>
              ...
              <ResponsiveContainer>
                {chartType === "line" ? (
                  <LineChart data={waterSeries}>
                    ...
                    <Line type="monotone" dataKey="water" stroke="#10b981" dot={{ r: 3 }} onClick={(e) => e?.payload && openDrilldownForDate(e.payload.date)} />
                  </LineChart>
                ) : (
                  <BarChart data={waterSeries}>
                    ...
                    <Bar dataKey="water" fill="#10b981" onClick={(e) => e?.payload && openDrilldownForDate(e.payload.date)} />
                  </BarChart>
                )}
              </ResponsiveContainer>
            </div>
              ...
              <ResponsiveContainer>
                {chartType === "line" ? (
                  <LineChart data={wasteSeries}>
                    ...
                    <Line type="monotone" dataKey="waste" stroke="#ef4444" dot={{ r: 3 }} onClick={(e) => e?.payload && openDrilldownForDate(e.payload.date)} />
                  </LineChart>
                ) : (
                  <BarChart data={wasteSeries}>
                    ...
                    <Bar dataKey="waste" fill="#ef4444" onClick={(e) => e?.payload && openDrilldownForDate(e.payload.date)} />
                  </BarChart>
                )}
              </ResponsiveContainer>
            </div>
              ...
              <ResponsiveContainer>
                {chartType === "line" ? (
                  <LineChart data={emissionsSeries}>
                    ...
                    <Line type="monotone" dataKey="emissions" stroke="#f59e0b" dot={{ r: 3 }} onClick={(e) => e?.payload && openDrilldownForDate(e.payload.date)} />
                  </LineChart>
                ) : (
                  <BarChart data={emissionsSeries}>
                    ...
                    <Bar dataKey="emissions" fill="#f59e0b" onClick={(e) => e?.payload && openDrilldownForDate(e.payload.date)} />
                  </BarChart>
                )}
              </ResponsiveContainer>
            </div>
      ...
      <DrilldownModal open={!!detail} onClose={() => setDetail(null)} title={detail?.title} rows={detail?.rows || []} />
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {tasks.map((t) => (
                <li key={t.id} className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="text-sm text-gray-700">{t.details?.message}</div>
                  </div>
                  <div className="flex flex-col gap-1">
                    <button
                      className="text-xs px-2 py-1 border rounded"
                      onClick={() => {
                        setTasks((prev) => prev.map((p) => (p.id === t.id ? { ...p, status: p.status === "Open" ? "Done" : "Open" } : p)));
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <div className="text-xs text-gray-500 mb-2">Recent actions (persisted)</div>
