...
export default function InsightsPage({ data, filteredData, totals }) {
  ...
  const series = useMemo(() => {
    if (metric === "overall") {
      ...
      const map = new Map();
      src.forEach(r => {
        if (!map.has(r.date)) map.set(r.date, {date: r.date, energy:0, water:0, waste:0, emissions:0});
  ...
  return (
    <div className="p-6">
      <button className="mb-4 px-2 py-1 border rounded" onClick={() => nav(-1)}>← Back</button>
      <h1 className="text-2xl font-bold mb-3">Insights — {metric?.toUpperCase()}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <BarChart width={300} height={200} data={hotspots.map(([k,v])=>({name:k, value:v}))}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="name" />
            <YAxis />
            <Tooltip />
      ...
      <div className="mt-4 bg-white p-4 rounded shadow">
        ...
        <ul className="list-disc pl-5">
          ...
          <li>Anomalies are flagged where values exceed threshold (you can refine thresholds).</li>
        </ul>
      </div>
    </div>
  );
